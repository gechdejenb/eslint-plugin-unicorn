...
const create = context => {
	...
	return {
		ForStatement(node) {
			...
			const firstIndex = node.update.range[0];
			const lastIndex = node.update.range[1];
			if(nodeUpdate){
				if (nodeUpdate.type === 'BinaryExpression' && nodeUpdate.operator !=='+'){
					return {
						loc: toLocation([firstIndex, lastIndex], sourceCode),
						messageId: 'no-for-loop',
					};
				}
				else if (nodeUpdate.type === 'AssignmentExpression' &&  !isLiteralOnePlusIdentifierWithName(nodeUpdate.right,indexIdentifierName)){
					return {
						loc: toLocation([firstIndex, lastIndex], sourceCode),
						messageId: 'no-for-loop',
					};
